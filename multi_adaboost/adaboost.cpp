...
AdaBoost::AdaBoost(vector<vector<double> > x, vector<int> y, vector<vector<double> > test, int m, int t1, int k1){
    ...
    uniform_int_distribution<int> dis(0, K-1);  //random initiation
    for(int i = 0; i < M; i++){
...
void AdaBoost::TrainProcessor(){
    for(int i = 0; i < T; i++){
        ...
        for(int i = 0; i < M; i++){
            ...
            for(int j = 0; j < K; j++){
                double s = 1;
                for(int t = 0; t < pnums / M; t++) s *= 1 / (1 + exp(-tmpS(candidate[t], j)));
...
void AdaBoost::SaveResult(){
    ...
    for(int i = 0; i < test_result.size(); i++) 
        outfile<<test_result[i]<<"  "<<scores[i][0]<<"  "<<scores[i][1]<<endl;
    outfile.close();
}
